...
class ARObservations(typing.NamedTuple):
    ...
    predictors: jax.Array  # (*B, R, L, D_y)
    targets: jax.Array  # (*B, R, D_y)
    ...
    def from_observations(
        ...
        if observations.ndim < 2:
            raise ValueError('observations must have shape (*B, T, D_y)')
        if num_lags < 1:
            raise ValueError('num_lags must be at least 1')
        if observations.shape[-2] <= num_lags:
        ...
        if predictors.shape != targets.shape[:-1] + (num_lags, targets.shape[-1]):
    ...
    def num_lags(self) -> int:
        ...
        return self.predictors.shape[-2]
